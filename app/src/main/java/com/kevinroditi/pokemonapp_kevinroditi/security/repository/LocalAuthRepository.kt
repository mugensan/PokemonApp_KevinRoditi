...
/**
 * Active authentication implementation for v2.5.
 * Uses local biometrics and encrypted shared preferences.
 */
class LocalAuthRepository @Inject constructor(
    ...
    override suspend fun authenticate(activity: FragmentActivity): Result<Unit> {
        return when (val biometricResult = authenticator.authenticate(activity)) {
